...
as well as from arcade.example:
http://arcade.academy/examples/tetris.html#tetris

NOTE: Copied from arcade.examples, debugged so the sprite textures actually show up.
Main problem was that sprite.textures must be set for textures to change, meaning each
sprite object needs the full list of textures defined. Otherwise the sprite will always 
show black instead of the correct colors.

Also did away with red bar on bottom
"""
...
def check_collision(board, shape, offset):
	...
	for cy, row in enumerate(shape):
		for cx, cell in enumerate(row):
			if cy + off_y >= ROW_COUNT or (cell and board[cy + off_y][cx + off_x]) :
				return True
	return False
